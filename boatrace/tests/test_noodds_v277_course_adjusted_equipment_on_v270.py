...
from train_noodds_v277_course_adjusted_equipment_on_v270 import (
    RAW_FEATURE_INDICES,
...
class CourseAdjustedEquipmentTests(unittest.TestCase):
    def test_fixed_interactions_and_missing_course_flag(self):
        ...
        self.assertEqual(RAW_FEATURE_INDICES, [9, 10, 95, 96, 97, 98, 99, 100])
        delta = course[0, :, 1]
        np.testing.assert_allclose(combined[0, :, 13], delta * 0.1)
        np.testing.assert_allclose(combined[0, :, 14], delta * 0.2)
        np.testing.assert_allclose(combined[0, :, 15], delta * -0.3)
        np.testing.assert_allclose(combined[0, :, 16], delta * -0.4)
        np.testing.assert_allclose(combined[0, :, 17], 1.0)
        np.testing.assert_allclose(combined[1, :, 17], 0.0)
