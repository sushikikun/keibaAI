...
SCRIPT = ROOT / "scripts" / "train_noodds_v295_large_history_exact_top3_with_proven_history_on_v283.py"
spec = importlib.util.spec_from_file_location("v295", SCRIPT)
v295 = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(v295)

def test_preregistration_is_frozen_and_time_safe():
    cfg = json.loads((ROOT / "configs" / "noodds_v295_large_history_exact_top3_with_proven_history_on_v283_preregistered.json").read_text(encoding="utf-8"))
    ...
    assert cfg["history_features"]["pairwise"]["pair_shrinkage"] == 20
    assert cfg["history_features"]["current_meet"]["window_days"] == 7

def test_remaining_candidates_respects_order_context():
    first = np.array([1, 4])
    second = np.array([3, 0])
    stage2 = v295.remaining_candidates(first)
    stage3 = v295.remaining_candidates(first, second)
...
def test_grouped_objective_has_zero_group_gradient_sum():
    objective = v295.grouped_softmax_objective(4)
...
def test_context_matrix_uses_selected_runner_and_differences():
    ...
    x, candidates = v295.build_context_matrix(block, np.array([2]), runner_width=2)
    assert x.shape == (5, 9)
...
def test_gamma_zero_reproduces_parent_exactly():
    ...
    got = v295.anchored_probability(parent, standalone, 0.0)
...
def test_fit_gamma_is_bounded_and_uses_given_oof_only():
    ...
    gamma, loss = v295.fit_gamma(parent, standalone, np.array([0,1,2]), [0.0,0.25])
...
def test_zero_context_models_produce_normalized_uniform_trifecta():
    ...
    got = v295.predict_contextual([ZeroModel(),ZeroModel(),ZeroModel()], block, np.array([0,1]), runner_width=3)
