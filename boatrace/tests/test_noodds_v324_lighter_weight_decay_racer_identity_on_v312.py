...
import train_noodds_v324_lighter_weight_decay_racer_identity_on_v312 as v324


def config():
    return json.loads(
        v324.PREREGISTRATION.read_text(encoding="utf-8")
    )
...
def test_preregistration_changes_only_weight_decay():
    ...
    assert cfg["training"]["weight_decay"] == 0.0003
...
def test_runtime_contract_preserves_parent_width_and_decay():
    cfg = v324.runtime_preregistration(config())
...
def test_train_predict_normalizes_and_records_regularization(tmp_path):
    ...
    probability, diagnostics = v324.train_and_predict(
        runner,
        race,
        identity,
        targets,
